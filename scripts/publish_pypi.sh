...
init_text = Path("src/compressibleSPH/__init__.py").read_text()
match = re.search(r'^__version__\s*=\s*"([^"]+)"', init_text, re.MULTILINE)
if match is None:
    print("Could not find __version__ in src/compressibleSPH/__init__.py", file=sys.stderr)
    sys.exit(1)
...
if pyproject_version != package_version:
    print(
        ...
        f"but src/compressibleSPH/__init__.py has {package_version}",
        file=sys.stderr,
    )
    sys.exit(1)
