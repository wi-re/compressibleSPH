#!/usr/bin/env python
"""06 periodic random flow -- thin wrapper around the shared runner.

The notebook this came from is `06-periodic-random-flow.ipynb`; the case itself is
...
plotting, ffmpeg) is `warpSPH.runner`. Equivalent invocations::
    ...
    python examples/weaklyCompressible/06-periodic-random-flow.py --no-bounded --obstacle --nx 128
...
PRESET = ['--no-bounded', '--obstacle', '--nx', '128']
