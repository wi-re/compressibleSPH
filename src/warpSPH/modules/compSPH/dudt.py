...
def computeCompSPHdudt_Func_i(
    ...
    for neighborIndex in range(numIndices):
        ...
        vTerm = -scalar_t(0.5) * apparentVolume * pi_i * mu_ij * dot
...
def computeCompSPHdudt_Kernel(
    ...
    out_dudt[i] = computeCompSPHdudt_Func_Adjacency(
        i, domainState.dim, 
...
def _compSPHdudtDtype(ctx, extras):
    return castTorchToWarpAsBuiltins(ctx.query.densities).dtype


_COMPSPH_DUDT = OperatorSpec(
    kernel=computeCompSPHdudt_Kernel,
    outputs=(OutputSpec(dtype=_compSPHdudtDtype, shape=ShapeOf.QUERY),),
    extras=(
...
def computeCompSPHdudtWarp(
    ...
    with record_function("warpSPH[computeCompSPHdudt]"):
        with record_function("warpSPH[computeCompSPHdudt] - Preprocessing"):
        ...
        with record_function("warpSPH[computeCompSPHdudt] - Kernel Execution"):
            ctx = SPHContext(
        ...
        #         launch_kernel, computeCompSPHdudt_Kernel, outputSize, outputDtype,
        #         *args,
