...
class KidderIsentropicCapsuleAnalyticSolution:
    ...
    def __init__(self,
        ...
        self.tau = math.sqrt(0.5*self.gamma1 * (r1*r1 - r0*r0)/(self.S*self.gamma*(self.rho1**self.gamma1 - self.rho0**self.gamma1)))
        assert self.tau > 0.0
    ...
    #---------------------------------------------------------------------------
    def hfrac(self, t):
        return math.sqrt(max(0.0, 1.0 - self.tfrac(t)**2))
    ...
    def accelr(self, t, r):
        hi = self.hfrac(t)
        return -r * (t**2 + self.tau**2) / (t**2 - self.tau**2)**2
