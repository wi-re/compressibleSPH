...
def computeM_Kernel(
    ...
    outputValues : wp.array(dtype = matrix(shape=(Any, Any), dtype=wp.float32)) # type: ignore
):                                                                                    
    i = wp.tid()
    ...
    outputValues[i] = computeM_Func_Adjacency(
        i, domainState.dim, 
...
def computeMWarp(
    ...
    with record_function("warpSPH[computeM]"):
        with record_function("warpSPH[computeM] - Preprocessing"):
            ...
            outputDtype = matrix(shape=(queryParticles.positions.shape[1], queryParticles.positions.shape[1]), dtype=wp.float32) # type: ignore
        ...
        #         launch_kernel, computeM_Kernel, outputSize, outputDtype,
        #         *args,
